      ...
      <div className="w-full flex justify-between items-center max-w-7xl mx-auto">
        <Link to="/" className="flex items-center gap-2"
          ...
          <p className="text-white text-[18px]
           font-bold cursor-pointer flex">Surya&nbsp;
            <span className="sm:block hidden">| Portfolio</span></p>
        </Link>
        <ul className='last-none hidden sm:flex flex-row gap-10'>
          {navLinks.map((link) => (
        ...
        <div className="sm:hidden flex flex-1 justify-end items-center">
          ...
          <div className={`${!toggle ? "hidden" : 'flex'} 
            ...
            <ul className='last-none flex justify-end items-start flex-col gap-4'>
              {navLinks.map((link) => (
                <li key={link.id} className={`${active === link.title ?
                  "text-white" : "text-secondary"} 
